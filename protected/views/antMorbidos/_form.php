<?php
/* @var $this AntMorbidosController */
/* @var $model AntMorbidos */
/* @var $form CActiveForm */
?>

<div class="form">

<?php $form=$this->beginWidget('CActiveForm', array(
	'id'=>'ant-morbidos-form',
	// Please note: When you enable ajax validation, make sure the corresponding
	// controller action is handling ajax validation correctly.
	// There is a call to performAjaxValidation() commented in generated controller code.
	// See class documentation of CActiveForm for details on this.
	'enableAjaxValidation'=>false,
)); ?>

	<p class="note">Fields with <span class="required">*</span> are required.</p>

	<?php echo $form->errorSummary($model); ?>

	<div class="row">
		<?php echo $form->labelEx($model,'id'); ?>
		<?php echo $form->textField($model,'id'); ?>
		<?php echo $form->error($model,'id'); ?>
	</div>

	<div class="row">
		<?php echo $form->labelEx($model,'hip_arterial'); ?>
		<?php echo $form->textField($model,'hip_arterial',array('size'=>45,'maxlength'=>45)); ?>
		<?php echo $form->error($model,'hip_arterial'); ?>
	</div>

	<div class="row">
		<?php echo $form->labelEx($model,'hip_colesterolemia'); ?>
		<?php echo $form->textField($model,'hip_colesterolemia',array('size'=>45,'maxlength'=>45)); ?>
		<?php echo $form->error($model,'hip_colesterolemia'); ?>
	</div>

	<div class="row">
		<?php echo $form->labelEx($model,'hipotiroidismo'); ?>
		<?php echo $form->textField($model,'hipotiroidismo',array('size'=>45,'maxlength'=>45)); ?>
		<?php echo $form->error($model,'hipotiroidismo'); ?>
	</div>

	<div class="row">
		<?php echo $form->labelEx($model,'barotrauma'); ?>
		<?php echo $form->textField($model,'barotrauma',array('size'=>45,'maxlength'=>45)); ?>
		<?php echo $form->error($model,'barotrauma'); ?>
	</div>

	<div class="row">
		<?php echo $form->labelEx($model,'diab_mellitus'); ?>
		<?php echo $form->textField($model,'diab_mellitus',array('size'=>45,'maxlength'=>45)); ?>
		<?php echo $form->error($model,'diab_mellitus'); ?>
	</div>

	<div class="row">
		<?php echo $form->labelEx($model,'enf_renal'); ?>
		<?php echo $form->textField($model,'enf_renal',array('size'=>45,'maxlength'=>45)); ?>
		<?php echo $form->error($model,'enf_renal'); ?>
	</div>

	<div class="row">
		<?php echo $form->labelEx($model,'trauma_ac_agudo'); ?>
		<?php echo $form->textField($model,'trauma_ac_agudo',array('size'=>45,'maxlength'=>45)); ?>
		<?php echo $form->error($model,'trauma_ac_agudo'); ?>
	</div>

	<div class="row">
		<?php echo $form->labelEx($model,'vibraciones'); ?>
		<?php echo $form->textField($model,'vibraciones',array('size'=>45,'maxlength'=>45)); ?>
		<?php echo $form->error($model,'vibraciones'); ?>
	</div>

	<div class="row buttons">
		<?php echo CHtml::submitButton($model->isNewRecord ? 'Create' : 'Save'); ?>
	</div>

<?php $this->endWidget(); ?>

</div><!-- form -->